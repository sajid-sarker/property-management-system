...
import { FaUser, FaHome, FaHeart, FaCog, FaSignOutAlt, FaPlus, FaBuilding, FaComments, FaBell } from 'react-icons/fa';
        ...
        <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--color-primary)' }}>
            ...
            <aside style={{ width: '280px', background: 'var(--color-primary-light)', borderRight: '1px solid rgba(255,255,255,0.05)', padding: '2rem' }}>
                ...
                <nav style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                    ...
                    <SidebarItem icon={<FaHome />} label="My Properties" active={activeTab === 'properties'} onClick={() => window.location.href = '/properties'} />
                    <SidebarItem icon={<FaBuilding />} label="Dev Requests" active={activeTab === 'dev-requests'} onClick={() => window.location.href = '/development-requests'} />
                    <SidebarItem icon={<FaHeart />} label="Wishlist" active={activeTab === 'wishlist'} onClick={() => window.location.href = '/wishlist'} />
                    <SidebarItem icon={<FaComments />} label="Messages" active={activeTab === 'messages'} onClick={() => setActiveTab('messages')} />
                    <SidebarItem icon={<FaBell />} label="Notifications" active={activeTab === 'notifications'} onClick={() => setActiveTab('notifications')} />
            ...
            <main style={{ flex: 1, padding: '3rem' }}>
                <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '3rem' }}>
                    <h1 style={{ fontFamily: 'var(--font-heading)', color: 'white' }}>Welcome back, <span className="text-accent">User</span></h1>
                ...
                <div style={{ background: 'var(--color-primary-light)', borderRadius: '12px', padding: '2rem', border: '1px solid rgba(255,255,255,0.05)' }}>
                    {activeTab === 'overview' && (
                        <>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                ...
                                <ActivityItem text="Message sent to Agent Sarah Jenkins" time="3 days ago" />
                            </div>
                        </>
                    )}
                    ...
                    {activeTab === 'notifications' && (
                        <>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                <ActivityItem text="Price Drop! 'Seaside Manor' is now $2.5M (was $2.8M)" time="1 hour ago" isHighlight />
                                <ActivityItem text="New Inquiry: Someone is interested in your listing" time="5 hours ago" />
                    ...
                    {activeTab === 'messages' && (
                        <>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                <div style={{ padding: '1rem', background: 'rgba(0,0,0,0.2)', borderRadius: '8px', borderLeft: '3px solid var(--color-accent)' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                                        <span style={{ fontWeight: 600 }}>Sarah Jenkins (Agent)</span>
                                        <span style={{ fontSize: '0.8rem', color: 'var(--color-text-light)' }}>2 mins ago</span>
                                    </div>
                                    <p style={{ color: 'var(--color-text-light)', fontSize: '0.9rem' }}>Hi! I'm available for a viewing of the Penthouse tomorrow at 2 PM.</p>
                                </div>
                                <div style={{ padding: '1rem', background: 'rgba(0,0,0,0.2)', borderRadius: '8px' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                                        <span style={{ fontWeight: 600 }}>John Doe (Buyer)</span>
                                        <span style={{ fontSize: '0.8rem', color: 'var(--color-text-light)' }}>1 day ago</span>
                                    </div>
                                    <p style={{ color: 'var(--color-text-light)', fontSize: '0.9rem' }}>Is the price negotiable for the Downtown Loft?</p>
                                </div>
                            </div>
                        </>
                    )}

    ...
    const Navbar = () => (
        <nav className={`navbar ${scrolled ? 'scrolled' : ''}`}>
            <div className="container nav-content">
                <a href="/" className="logo">Luxe<span className="text-accent">Estate</span></a>
                <div className="nav-links hidden-mobile">
                    <a href="#featured" className="nav-link">Residences</a>
                    <a href="#about" className="nav-link">The Agency</a>
        ...
        <div className="home-page" style={{ background: 'var(--color-primary)' }}>
            ...
            <section className="hero-section" style={{
                ...
                <div className="hero-bg" style={{
                    ...
                    backgroundImage: 'url("https://images.unsplash.com/photo-1622372738946-a2e5330eef76?q=80&w=2574&auto=format&fit=crop")',
                ...
                <motion.div
                    ...
                    style={{ position: 'relative', zIndex: 10, textAlign: 'center', width: '100%', maxWidth: '900px', y: yHero }}
                >
                    <motion.div
                        initial={{ opacity: 0, letterSpacing: '10px' }}
                        animate={{ opacity: 1, letterSpacing: '3px' }}
                        transition={{ delay: 0.5, duration: 1 }}
                        style={{ textTransform: 'uppercase', color: 'var(--color-accent)', marginBottom: '1rem', fontSize: '0.875rem' }}
                    >
                        Exclusive Living
                    </motion.div>
                    <h1 style={{ fontSize: 'clamp(3rem, 5vw, 5rem)', marginBottom: '2rem', textShadow: '0 4px 20px rgba(0,0,0,0.5)' }}>
                        Curating the World's <br /> <span style={{ fontStyle: 'italic', fontFamily: 'serif' }}>Finest Properties</span>
                    </h1>
                    ...
                    >
                        <input type="text" placeholder="Search by location, property type..." style={darkInputStyle} />
            ...
            <section id="featured" className="featured-section" style={{ padding: '8rem 0', background: 'var(--color-primary)' }}>
                <div className="container">
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '4rem' }}>
                        <div>
                            <h4 className="text-accent" style={{ textTransform: 'uppercase', letterSpacing: '2px', fontSize: '0.875rem', marginBottom: '1rem' }}>The Collection</h4>
                            <h2 style={{ fontSize: '3rem', color: 'var(--color-text-main)' }}>Featured Residences</h2>
                        </div>
                        <button className="btn btn-outline">View All Listings</button>
                    </div>

                    {loading ? (
                        <div style={{ color: 'white', textAlign: 'center' }}>Loading Collection...</div>
                    ) : (
            ...
            <section id="services" style={{ padding: '8rem 0', background: 'var(--color-primary-light)' }}>
                <div className="container" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4rem', alignItems: 'center' }}>
                    <div>
                        <h2 style={{ fontSize: '3rem', marginBottom: '2rem' }}>Experience the <span className="text-accent" style={{ fontStyle: 'italic' }}>Extraordinary</span></h2>
                        <p style={{ color: 'var(--color-text-light)', fontSize: '1.1rem', marginBottom: '2rem', lineHeight: '1.8' }}>
                            We don't just find you a house; we curate a lifestyle. Our team of elite real estate professionals provides an unparalleled level of service, ensuring your journey is as exceptional as your destination.
                        </p>
                        <ul style={{ listStyle: 'none', display: 'grid', gap: '1.5rem' }}>
                            ...
                            <li style={{ display: 'flex', alignItems: 'center', gap: '1rem', fontSize: '1.1rem' }}>
                                <FaStar className="text-accent" /> Global Investment Portfolio Management
                            </li>
                        </ul>
                    </div>
                    <div style={{ position: 'relative' }}>
                        <img
                            ...
                            alt="Luxury Interior"
                            style={{ width: '100%', borderRadius: '4px', filter: 'brightness(0.9)' }}
                        />
                        <div style={{
            ...
            {/* Footer */}
            <footer style={{ background: 'var(--color-background)', padding: '6rem 0 2rem 0', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                <div className="container" style={{ textAlign: 'center' }}>
                    <h2 className="logo" style={{ marginBottom: '2rem' }}>Luxe<span className="text-accent">Estate</span></h2>
                    ...
                    <div style={{ color: 'var(--color-text-light)', fontSize: '0.875rem' }}>
                        &copy; 2024 LuxeEstate Property Management. Created with React & MERN Architecture.
                    </div>
                </div>
            </footer>
        </div>
...
const PropertyCard = ({ data, index }) => (
    <motion.div
        ...
        whileHover={{ y: -10 }}
        style={{ background: 'var(--color-primary-light)', borderRadius: '4px', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.05)' }}
    >
        <div style={{ height: '300px', overflow: 'hidden', position: 'relative' }}>
            <img src={data.image} alt={data.title} style={{ width: '100%', height: '100%', objectFit: 'cover', transition: 'transform 0.7s ease' }} />
            <div style={{
                position: 'absolute', bottom: 0, left: 0, right: 0,
                background: 'linear-gradient(to top, rgba(0,0,0,0.8), transparent)',
                padding: '2rem 1.5rem 1rem 1.5rem'
            }}>
                <div style={{ color: 'var(--color-accent)', fontWeight: 600, fontSize: '1.25rem', marginBottom: '0.25rem' }}>{data.price}</div>
                <h3 style={{ fontSize: '1.5rem', marginBottom: '0.5rem' }}>{data.title}</h3>
            </div>
        </div>
